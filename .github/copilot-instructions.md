...
> 本文件是 `.claude/docs/` 子文件的**合并完整版**，供 Copilot/Codex 使用。
> **主源**在 `.claude/docs/`，修改规范时先改主源，再合并到此文件。
> 另需同步更新：`.cursor/rules/project-rules.md`（Cursor 版）
...
| `.github/copilot-instructions.md` | Copilot、Codex | 合并完整版（本文件） |

**同步策略**：先修改 `.claude/docs/` 子文件（主源），再合并到本文件和 Cursor 版本。
