...
| 缺陷 | 成因 | 缓解方案 |
|------|------|---------|
| 鬼影/拖影 | 运动向量不准确，历史颜色重投影错误 | 使用更精确的运动向量（物体运动 + 相机运动）；邻域裁剪（Neighborhood Clamping）限制历史颜色范围 |
| 细节丢失/模糊 | 历史帧加权平均模糊高频细节 | 增加当前帧权重；使用锐化滤波器（如 CAS）增强细节；高频区域使用更小历史权重 |
| 闪烁 | 小物体或细线条在亚像素抖动时闪烁 | 增加历史缓冲长度；响应式权重；闪烁区域使用更大历史权重 |
| 透明物体问题 | 透明物体运动向量难以准确计算 | 透明物体不参与 TAA，使用 FXAA；或使用专用透明物体运动向量计算方法 |

**工程落地补充**：
- TAA 是 3A 游戏和高端手游的标配，性能开销低、画质收益高
- 必须配合反向 Z 使用，否则历史帧深度重投影精度有问题
- 历史缓冲格式建议使用 R16G16B16A16F 获得更好精度
- 当前帧与历史帧的混合权重通常为 0.1（当前）: 0.9（历史），可根据场景动态调整
