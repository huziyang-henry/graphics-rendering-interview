...
- 每个变换由一个4x4矩阵完成，最终通过矩阵级联（MVP = Projection * View * Model）在一次乘法中完成所有变换。
...
- 由平移（Translation）、旋转（Rotation）、缩放（Scaling）组合而成：M = T * R * S
...
- View Matrix = Inverse(CameraWorldMatrix)，即相机世界变换矩阵的逆矩阵
...
- 透视投影：近大远小效果，通过w分量实现（w = -z_view），远处的物体在透视除法后变小
- 正交投影：无近大远小效果，w恒为1，平行线保持平行
- 透视投影的关键参数：FOV（视场角）、Aspect（宽高比）、Near、Far

### 裁剪空间 → NDC → 屏幕空间

- 透视除法：NDC = (x/w, y/w, z/w)，将齐次坐标转换为标准化设备坐标
- NDC是一个[-1,1]的立方体（OpenGL）或[0,1]的立方体（DirectX的z范围）
- 屏幕映射：ScreenX = (NDC.x + 1) * 0.5 * ViewportWidth, ScreenY = (1 - NDC.y) * 0.5 * ViewportHeight（Y轴翻转）
- 最终得到的屏幕坐标用于光栅化阶段的三角形遍历


## 🛠 工程实践

- 矩阵预乘优化：在CPU端预先计算 MVP = Projection * View * Model，在VS中只需一次矩阵乘法。对于静态物体，Model矩阵不变时可以缓存MVP结果。
...
- 法线矩阵（Normal Matrix）是Model矩阵的逆转置的3x3子矩阵：normalMatrix = transpose(inverse(mat3(modelMatrix)))。仅在存在非均匀缩放时才需要，均匀缩放可以用原矩阵的3x3子矩阵代替。
...
- 矩阵乘法顺序错误：MVP = P * V * M（从右到左应用），如果写成 M * V * P 则结果完全错误。这是因为矩阵乘法不满足交换律。
